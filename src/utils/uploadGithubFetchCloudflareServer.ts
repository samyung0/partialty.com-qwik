...
export const uploadGithubFetchCloudflare = server$(async function (
  ...
  for (let i = 0; i < files.length; i++) {
    try {
      const _contentType = contentType(files[i].path.slice(files[i].path.lastIndexOf('.') + 1).toLowerCase());
  ...
  const getFulfilled = <T>(p: PromiseSettledResult<T>, index: number): { index: number; value: T } | null =>
    p.status === 'fulfilled' ? { index, value: p.value } : null;
  const getRejected = <T>(p: PromiseSettledResult<T>, index: number): { index: number; reason: string } | null =>
  ...
  const failedFiles = mamoru.map(getRejected).filter((p): p is { index: number; reason: string } => p !== null);

  return [
    true,
    failedFiles.reduce((prev, curr) => prev + 'Failed. ' + files[curr.index].path + ' ' + curr.reason + '\n', '') +
