...
const QuizHydrate = ({
  ...
  useEffect(() => {
    (Array.from(document.getElementsByClassName("quizOptionButton")) as HTMLElement[]).forEach(
      (button) => {
        button.addEventListener("click", (e) => {
          ...
          const quizOption = button.getElementsByClassName("quizOption")[0] as
            | HTMLElement
            | undefined;
          if (!quizOption) return;
    ...
    (Array.from(document.getElementsByClassName("quizBlock")) as HTMLFormElement[]).forEach(
      (form) =>
