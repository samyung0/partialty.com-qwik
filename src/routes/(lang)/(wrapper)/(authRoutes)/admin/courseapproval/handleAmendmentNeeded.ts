...
export default $(
  async (event: RequestEventBase, userRole: string, approvalId: string, need_amendment_reason: string) => {
    ...
    await drizzleClient(event.env, import.meta.env.VITE_USE_PROD_DB === '1').transaction(async (tx) => {
      await tx
        ...
        .set({ status: 'need_amendment', updated_at: getSQLTimeStamp(), need_amendment_reason })
